...
TEST(IntegrationTests, assertTest){
    constexpr double start = 0;
    constexpr double end = 10;
    constexpr double it = 1;
    constexpr double acc = 0;

    std::cout<< integrate<double(double), 3>(std::sin, start, end, it, acc)<<"\n";
...
TEST(IntegrationTests, firstTest){
    ...
    for(auto&& it: dx){
        fileOut3<< std::abs(answ - integrate<double(double), 3>(std::sin, start, end, it))<<"\n";
    }
    for(auto&& it: dx){
        fileOut5<< std::abs(answ - integrate<double(double), 5>(std::sin, start, end, it))<<"\n";
    }
    fileOut3.close();
    fileOut5.close();
